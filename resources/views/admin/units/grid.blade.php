...
{{--  Search   --}}
<div class="row">
    <form class="m-form" action="{{route('units.index')}}" method="get" id="search_form">
    ...
    @if(request()->search)
        <div class="col-2">
            <button class="btn btn-success m-btn m-btn--custom m-btn--icon m-btn--pill m-btn--air" onclick="window.location.href='{{route('units.index')}}'">
                <span >
                ...
                <div class="m-portlet__head-wrapper">
                    ...
                    @if(Auth::user()->permissions->can_create)
                        <div class="m-portlet__head-tools">
                            <a href="{{ route('units.create') }}" class="btn btn-brand m-btn m-btn--icon m-btn--wide m-btn--md m--margin-right-10">
                                <span>
            ...
            <div class="filter-div">
                <div class="row">
                    <div class="col-12">
                        ...
                        <select class="form-control m-input filter-status" data-search_parameter="building" data-current-url="{{route('units.index')}}">
                ...
                <div class="m-portlet__foot m-portlet__foot--fit" style="margin-top: 12px;">
                    <div class="m-form__actions m-form__actions--solid">
                        <div class="row" style="margin-top: 5px;">
                            <div class="col-9">
                                <button class="btn btn-brand createFilter" data-current-url="{{route('units.index')}}">{{ trans('admin.create') }}</button>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <table class="table table-bordered" id="myTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th><b>@lang('admin.buildings')</b></th>
                                                ...
                                                <th scope="row">
                                                    @if(Auth::user()->permissions->can_show)
                                                        <div data-toggle="tooltip" data-placement="bottom" title="{{trans('admin.show_details')}}" class="btn-group mr-2" role="group" aria-label="First group">
                                                            <a type="button" href="{{ route('units.show', $unit->id) }}" class="m-btn m-btn m-btn--square btn btn-secondary">
                                                    ...
                                                    @if(Auth::user()->permissions->can_edit)
                                                        <div data-toggle="tooltip" data-placement="bottom" title="{{trans('admin.edit')}}" class="btn-group mr-2" role="group" aria-label="First group">
                                                            <a type="button" href="{{ route('units.edit', $unit->id) }}" class="m-btn m-btn m-btn--square btn btn-secondary">
                                                    ...
                                                    @if(Auth::user()->permissions->can_delete)
                                                        <div data-toggle="tooltip" data-placement="bottom" title="{{trans('admin.delete')}}" class="btn-group mr-2 delete-btn" data-delete-url="{{route('units.destroy',$unit->id)}}" role="group" aria-label="First group">
