...
{{--  Search   --}}
<div class="row">
    <form class="m-form" action="{{route('emergency-services.index')}}" method="get" id="search_form">
    ...
    @if(request()->search)
        <div class="col-2">
            <button class="btn btn-success m-btn m-btn--custom m-btn--icon m-btn--pill m-btn--air" onclick="window.location.href='{{route('emergency-services.index')}}'">
                <span >
                ...
                <div class="m-portlet__head-wrapper">
                    ...
                    @if(Auth::user()->permissions->can_create)
                        <div class="m-portlet__head-tools">
                            <a href="{{ route('emergency-services.create') }}" class="btn btn-brand m-btn m-btn--icon m-btn--wide m-btn--md m--margin-right-10">
                                <span>
                                    ...
                                    @else 
                                        @foreach($services as $index => $service)
                                            <tr>
                                                ...
                                                <th scope="row">{{$service->title_en}}</th>
                                                <th scope="row">
                                                    @if($service->status)
                                                        {{ trans('admin.active') }}
                                                    @else
                                                ...
                                                <th scope="row">
                                                    @if(Auth::user()->permissions->can_show)
                                                        <div data-toggle="tooltip" data-placement="bottom" title="{{trans('admin.show_details')}}" class="btn-group mr-2" role="group" aria-label="First group">
                                                            <a type="button" href="{{ route('emergency-services.show', $service->id) }}" class="m-btn m-btn m-btn--square btn btn-secondary">
                                                    ...
                                                    @if(Auth::user()->permissions->can_edit)
                                                        <div data-toggle="tooltip" data-placement="bottom" title="{{trans('admin.edit')}}" class="btn-group mr-2" role="group" aria-label="First group">
                                                            <a type="button" href="{{ route('emergency-services.edit', $service->id) }}" class="m-btn m-btn m-btn--square btn btn-secondary">
                                                    ...
                                                    @if(Auth::user()->permissions->can_delete)
                                                        <div data-toggle="tooltip" data-placement="bottom" title="{{trans('admin.delete')}}" class="btn-group mr-2 delete-btn" data-delete-url="{{route('emergency-services.destroy',$service->id)}}" role="group" aria-label="First group">
