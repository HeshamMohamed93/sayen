...
{{--  Search   --}}
<div class="row">
    <form class="m-form" action="{{route('contact-us.index')}}" method="get" id="search_form">
    ...
    @if(request()->search)
        <div class="col-2">
            <button class="btn btn-success m-btn m-btn--custom m-btn--icon m-btn--pill m-btn--air" onclick="window.location.href='{{route('contact-us.index')}}'">
                <span >
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered" id="myTable">
                                <thead>
                                    <tr>
                                        <th><b>#</b></th>
                                        <th><b>@lang('admin.sender_name')</b></th>
                                        <th><b>@lang('admin.phone')</b></th>
                                        <th><b>@lang('admin.email')</b></th>
                                        <th><b>@lang('admin.message')</b></th>
                                        <th><b>@lang('admin.send_from')</b></th>
                                        <th><b>@lang('admin.send_date')</b></th>
                                    ...
                                    @else 
                                        @foreach($messages as $index => $message)
                                            <tr>
                                                ...
                                                <th scope="row">@if(isset($message->user)){{$message->user->phone}}@endif</th>
                                                <th scope="row">{{$message->email}}</th>
                                                <th scope="row">{{str_limit($message->message, 50)}}</th>
                                                <th scope="row">
                                                    @if($message->send_from == 1)
                                                        {{trans('admin.app')}}
                                                    @else 
                                                        {{trans('admin.web')}}
                                                    @endif
                                                ...
                                                <th scope="row">
                                                    @if(Auth::user()->permissions->can_show)
                                                        <div data-toggle="tooltip" data-placement="bottom" title="{{trans('admin.show_details')}}" class="btn-group mr-2" role="group" aria-label="First group">
                                                            <a type="button" href="{{ route('contact-us.show', $message->id) }}" class="m-btn m-btn m-btn--square btn btn-secondary">
