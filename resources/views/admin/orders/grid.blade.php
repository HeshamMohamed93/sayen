...
{{--  Search   --}}
<div class="row">
    <form class="m-form" action="{{route('orders.index')}}" method="get" id="search_form">
        <div class="form-group m-form__group row ">
            ...
            <div class="col-6">
                ...
                @endif
                @if(request()->order_service)
                    <input type="hidden" name="order_service" class="form-control m-input" value="{{request()->order_service}}" >
                @endif
    ...
    @if(request()->search)
        <div class="col-2">
            <button class="btn btn-success m-btn m-btn--custom m-btn--icon m-btn--pill m-btn--air" onclick="window.location.href='{{route('orders.index')}}'">
                <span >
            ...
            <div class="m-portlet__head">
                <div class="m-portlet__head-wrapper">
                    ...
                    <div class="m-portlet__head-tools">
                        <a data-removeTestOrder-url="{{url('admin-panel/remove-test-orders')}}" class="removeTestOrder btn btn-danger m-btn m-btn--icon m-btn--wide m-btn--md m--margin-right-10">
                            <span>
                                <i class="la la-remove"></i>
                                <span>@lang('admin.remove_test_orders')</span>
                            </span>
                        </a>
                    </div>
                    {{--  Filter  --}}
            ...
            <div class="filter-div">
                <div class="row">
                    ...
                    <div class="col-6">
                        ...
                        <input type="date" class="form-control m-input date_to filter-status" data-search_parameter="date_to" data-current-url="{{route('orders.index')}}" name="date_to" value="{{request()->date_to}}">
                    </div>
                    <div class="col-3">
                        <label>{{trans('admin.status')}}</label>
                        <select class="form-control m-input filter-status" data-search_parameter="order_type" data-current-url="{{route('orders.index')}}">
                    ...
                    <div class="col-3">
                        ...
                        <select class="form-control m-input filter-status user_id" data-search_parameter="order_client" data-current-url="{{route('orders.index')}}">
                            <option value="0">{{ trans('admin.all') }}</option> 
                    ...
                    <div class="col-3">
                        ...
                        <select class="form-control m-input filter-status" data-search_parameter="order_team" data-current-url="{{route('orders.index')}}">
                            <option value="0">{{ trans('admin.all') }}</option> 
                    ...
                    <div class="col-3">
                        ...
                        <select class="form-control m-input filter-status" data-search_parameter="order_service" data-current-url="{{route('orders.index')}}">
                            <option value="0">{{ trans('admin.all') }}</option> 
                ...
                <div class="m-portlet__foot m-portlet__foot--fit">
                    <div class="m-form__actions m-form__actions--solid">
                        <div class="row">
                            <div class="col-9">
                                <button class="btn btn-brand createFilter" data-current-url="{{route('orders.index')}}">{{ trans('admin.create') }}</button>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <table class="table table-bordered datatables" id="myTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th><b>{{ trans('admin.building') }}</b></th>
                                        <th><b>{{ trans('admin.flat') }}</b></th>
                                    ...
                                    @else 
                                        @foreach($orders as $index => $order)
                                            <tr>
                                                ...
                                                <th scope="row">
                                                    <a href="{{ url('admin-panel/users') }}/{{ $order->orderUser->id }}/edit" target="_blank">
                                                        {{$order->orderUser->name .' '.$order->orderUser->last_name}}
                                                    </a>
                                                    <br>
                                                    @if($order->orderUser->excellence_client == 1)
                                                    {{ ($order->orderUser->excellence_client_verified )?trans('admin.excellence_client_yes'):trans('admin.excellence_client_no') }}
                                                    @endif
                                                </th>
                                                <th scope="row">{{($order->orderUser->building)?$order->orderUser->building->name:''}}</th>
                                                <th scope="row">{{$order->orderUser->flat}}</th>
                                                ...
                                                <th scope="row">{{$order->orderPayMethod()}}</th>
