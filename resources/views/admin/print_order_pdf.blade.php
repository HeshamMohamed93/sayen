    ...
    <body>
        <div class="full A5">
    		<div class="header">
    			...
    			<div class="info_ar">
    				<div class="col-6">
						<div class="doc-name">{{ trans('admin.order_up') }}</div>
					</div>
    			</div>
    		</div>
    		<div class="patient">
    			<table style="width:100%; text-align: left; border-collapse: collapse; margin:0 auto; border:1px solid #000;">
					<tr style=" border:1px solid #000;">
						<th colspan="2" style="text-align: center; border:1px solid #000;">{{ $order->order_number }}</th>
    					<td colspan="2" style="text-align: center; border:1px solid #000;"> {{trans('admin.order_number')}}</td>
    				</tr>
					<tr style=" border:1px solid #000;">
						<th style=" border:1px solid #000;">{{ implode(",",$service) }}</th>
    					<td style=" border:1px solid #000;">{{trans('admin.type_service')}}</td>
    					<th style=" border:1px solid #000;"> @if($order->visit_date) {{date('H:i:s',strtotime($order->visit_date))}} @endif </th>
    					<td style=" border:1px solid #000;">{{ trans('admin.start_work') }}</td>
    				</tr>
    				<tr style=" border:1px solid #000;">
    					<th style=" border:1px solid #000;"></th>
    					<td style=" border:1px solid #000;"></td>
						<th style=" border:1px solid #000;">@if($order->visit_date) {{date('Y-m-d',strtotime($order->visit_date))}} @endif</th>
    					<td style=" border:1px solid #000;">{{ trans('admin.date') }}</td>
    				</tr>
    				<tr style=" border:1px solid #000;">
                        <th style=" border:1px solid #000;"></th>
    					<td style=" border:1px solid #000;"></td>
    					<th style=" border:1px solid #000;">{{$building}}</th>
    					<td style=" border:1px solid #000;">{{ trans('admin.building') }}</td>
    				</tr>
                    <tr style=" border:1px solid #000;">
                        <th style=" border:1px solid #000;"></th>
    					<td style=" border:1px solid #000;"></td>
    					<th style=" border:1px solid #000;">{{$flat}}</th>
    					<td style=" border:1px solid #000;">{{ trans('admin.flat') }}</td>
    				</tr>
					<tr>
						<td></td>
						<td></td>
						<td>@if($client && $client->building) {{$client->building->owner_name}} @endif</td>
						<td>المالك</td>
					</tr>
    			</table>
    		</div>
				...
				<div class="Signature2">
					<div class="box">
						<div class="head-title">
							...
							<p>{{ $workDetails }}</p>
							<h4>{{ trans('admin.client') }} : {{ $client->name }}</h4>
							<h4>{{ trans('admin.phone') }} : {{ $client->phone }}</h4>
					...
					<div class="box" style="padding-bottom:30px">
						<div class="head-title">
							...
							<p>الفني : {{isset($order->orderTeam->name)? $order->orderTeam->name: trans('admin.no_assign_team')}}</p>
							<div class="line">
