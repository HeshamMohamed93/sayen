...
{
    // === Return buildings to user app ===
    public function buildings()
    {
        $buildings = Building::orderBy('name','ASC')->get();
        foreach($buildings as $building){
            $building['units'] = $building->getNameUnits();
       }
        return $this->respond(['buildings' => $buildings, 'status_code' => 200]);
    }
    // === End function ===
            ...
            else
            {
                ...
                else
                {
                    ...
                    if($request->excellence_client == 1)
                    {
                        ...
                        {
                            return $this->respond(['message' => trans('api.select_building'), 'status_code' => 400]);
                        }
                        if($request->flat == null)
                        {
                            return $this->respond(['message' => trans('api.enter_flat'), 'status_code' => 400]);
                        }
                        else
                        {
                            if(strlen($request->flat) == 1 ){
                                $flat = '00'.$request->flat;
                            }elseif(strlen($request->flat) == 2){
                                $flat = '0'.$request->flat;
                            }else{
                                $flat = $request->flat;
                            }
                            $request['flat'] = $flat;
                            $BuildName = Building::find($request->building_id)->name;
                            
                            $unit = Unit::where('name', '=', "$BuildName/$flat")->where('building_id', '=', $request->building_id)->get();
                            if(!$unit || $unit->count()==0)
                            {
                              return $this->respond(['message' => trans('api.wrong_flat'), 'status_code' => 400]);
                            }
                        }
                    }
                    else
                    ...
                    {
                        $create_otp = $this->createOTP($save_user->id, '1', '1', trans('api.activation_code_msg'),$lang);
    ...
    public function forgetPassword(Request $request)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if($this->user_status == true)
                {
                    $create_otp = $this->createOTP($member->id, $request->user_type, '2', trans('api.reset_password_code_msg'),$lang);
    ...
    public function createPlayerID(Request $request)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if($this->user_status == true)
                {
                    $set_player_id = DB::table($table)->where('id', $member->id)->update(array('player_id' => $request->player_id,'device_lang' => $request->header('lang')));
                    return $this->respond(['message' => trans('api.suucess_update') ,'status_code' => 200]);
                }
                else
                {
        ...
        else
        {
            ...
            else
            {
                ...
                else if($request->code_type == 3)  //=== change phone
                {
                    ...
                    {
                        $create_otp = $this->createOTP($member->id, $request->user_type, $request->code_type, trans('api.change_phone_code'),$lang);
                        if($create_otp)
                        {
                            return $this->respond(['message' => trans('api.code_is_sent'), 'status_code' => 200]);
                        }
                ...
                if($member)
                {
                    ...
                    else
                    {
                        ...
                        if($this->user_status == true || ($member->phone_verified == 0 && $request->code_type == 1) || ($member->phone_verified == 0 && $request->code_type == 2))
                        {
                            ...
                            {
                                //dd($member->id, $request->user_type, $request->code_type, trans('api.change_phone_code'));
                                $create_otp = $this->createOTP($member->id, $request->user_type, $request->code_type, $message,$lang);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    $create_otp = $this->createOTP($member->id, $request->user_type, '3', trans('api.change_phone_code'),$lang);
    ...
    public function profile(Request $request)
    {
        ...
        if($member)
        {
            ...
            {
                $member['building_name'] = ($member->building)?$member->building->name:'';
            }
    ...
    // === Create OTP code ===
    public function createOTP( $id, $member_type, $code_type, $message,$lang)
    {
        $existing_member;
        app()->setLocale($lang);
    ...
    public function createOTPUser(Request $request){
        $lang = app()->setLocale($request->header('lang'));
        $user = User::find($request->id);
        $create_otp = $this->createOTP($user->id, '1', '2', trans('api.change_password_code_msg'),$lang);
        $code = VerifyOTP::where('user_id',$user->id)->where('user_type','1')->where('code_type','2')->orderBy('id','DESC')->first();
        return $this->respond(['message' => trans('api.code_is_sent'), 'code' => $code->code,'status_code' => 200]);
    }
}
